...
export default function shoppingList() {
  ...
  const addOrSave = async () => {
    ...
    if (editingId) {
      await updateDoc(doc(db, "items", editingId), { text: trimmed });
      setEditingId(null);
    } else {
  ...
  return (
    <View style={s.container}>
      <Text style={s.title}>Your Shopping List</Text>
      <View style={s.row}>
        <TextInput
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={s.subtle}>No shopping items have been added</Text>
        }
      />
    </View>
  );
